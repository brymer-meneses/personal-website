...
function BlogPreview(props: props) {
  ...
  const [attributes, setAttributes] = useState<BlogAttribute>({ created: "", tags: "", title: "" });
  ...
  return (
      <div className="blog-preview-item" onClick={()=> navigate(`/blog/${props.filename}`)}>
        <h2>{attributes.title}</h2>
        <p>{attributes.created}</p>
        <div className="blog-preview-item__tags">
          { attributes.tags.split(",").map((tag, i)=> { return <p key={i}>#{tag.trim()}</p> }) }
        </div>
      </div>
  )
}
